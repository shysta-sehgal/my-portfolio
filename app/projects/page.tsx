...
export default function ProjectsPage() {
  ...
  const toggleProject = (key: string) => {
    ...
    const isOpening = !newOpenProjects.has(key);
    
    if (isOpening) {
      newOpenProjects.add(key);
      // Only scroll when opening a project
      setTimeout(() => {
          ...
          <section key={year} className="mb-10">
            ...
            {projects.map((project, index) => {
              ...
              return (
                <div
                  key={key}
                  className="border-b border-gray-700 py-4 scroll-mt-24"
                  ref={(el) => {
                    refs.current[key] = el;
                  }}
                >
                  <button
                    className="w-full text-left text-xl font-semibold hover:text-rose-400 transition flex items-center justify-between"
                      ...
                      <div className="space-x-4">
                        ...
                        {project.paper && (
                          <>
                            ...
                            <iframe
                              src={project.paper}
                              className="hidden md:block w-full h-[700px] rounded-lg shadow-lg border mt-4"
                            />
                          </>
                        )}
                        {project.slides && (
